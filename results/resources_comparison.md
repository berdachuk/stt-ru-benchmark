...
| gemma4-e4b-8bit (bnb) | ~11269 MiB | ~4.5 GiB free; WER ruined |
...
### FLEURS n=50 (12b-qat: n=48, 2 API errors)
...
- **Best multimodal so far: `gemma4-12b-qat` (Ollama QAT Q4_0 GGUF)** — FLEURS ≈ Speaches/GigaAM (CI overlap); Podlodka ~+3 п.п. vs GigaAM (significant) but much better than HF bnb4 12B.
- **E4B QAT (HF+bnb4)** usable (~7–12% WER), far better than vanilla bnb 8-bit (63%).
- **Vanilla bnb 8/4-bit on non-QAT checkpoints** damages ASR badly; prefer official QAT GGUF / QAT+bnb4.
- Production still: **GigaAM** (speed + Podlodka) / **Speaches** (FLEURS). QAT 12B is the first multimodal close enough to consider as a single multimodal stack.
